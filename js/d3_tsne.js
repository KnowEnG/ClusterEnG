...
function d3tsne(file, id, algorithm, xycode) {
	...
	$('#'+id).append('<div class="row" id="' + algorithm + '-tsnePlots" tabindex="0"></div>')
	...
	//tsne components and their columns index

	var tsne= [[1,2],[1,3],[2,3]];
...
*/
	...
	function currentBrush() {
		...
		svg[plot_number].selectAll('circle').classed('hidden', function(d) {
			...
			plotx=tsne[plot_number][0];ploty=tsne[plot_number][1];
	...
	function ready(error,Data) {
		if (error) throw error;
			if(d3.keys(Data[0]).length <= 5){
				drawChart(Data,tsne[0][0],tsne[0][1]);
			}
			else{
				drawChart(Data,tsne[0][0],tsne[0][1]);
		 		drawChart(Data,tsne[1][0],tsne[1][1]);
		 		drawChart(Data,tsne[2][0],tsne[2][1]);
	...
	var drawChart = function(data,plotx,ploty) {
		...
		svg[n] = d3.select('#' + algorithm + '-tsnePlots').append('svg')
		...
		circle[n] = points_svg[n].selectAll('circle').data(data)
			.enter().append('circle')
				...
				.attr('cx', function(d) {
					return x(width/2+spacing*normalize(parseFloat(d[axis_labels[n][plotx]])))})
				.attr('cy', function(d) {
					return y(height/2+spacing*normalize(parseFloat(d[axis_labels[n][ploty]])))})
		...
		var xlabel=function(){
			...
			return "t-SNE"+ (tsne[n][0]);
		}
		var ylabel=function(){
			return "t-SNE"+ (tsne[n][1]);
		}
...
*/
	...
	d3.select("#" + algorithm + "-tsnePlots").on("keydown", keyDown);
	function keyDown() {
	...
	d3.select("#" + algorithm + "-tsnePlots").on("keyup", keyUp);
	function keyUp() {
		d3.event.preventDefault();
		if(fo===true){
